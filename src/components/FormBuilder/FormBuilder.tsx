...
export default class FormBuilder<T> {
  ...
  withInput = (props: { name: string; type?: string; label: string }) => {
    this.components.push(formikProps => {
      const isInvalid = this.isInvalid(props.name, formikProps);
      return (
        <InputContainer>
  ...
  private isInvalid = (name: string, props: FormikProps<T>) =>
    Boolean((props.touched as any)[name] && (props.errors as FormErrors)[name]);
}
